...
one YAML entry, runs this script, and the README stays consistent. The table
is ordered by the `rank` field so the README reflects the ranking in one place.
"""
...
def build_table(tools: list[dict]) -> str:
    ordered = sorted(tools, key=lambda t: t.get("rank", 999))
